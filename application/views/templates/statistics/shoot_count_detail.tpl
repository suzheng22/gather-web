        ...
        <div class="viewport_main">
        	<div class="rose_box cc_box">
                ...
                <div class="rose_top main_rignt_top clearfix">
                        ...
                        <div class="cc_top_one"><label>拍摄人:</label><span>{{$uInfo.userName}}</span></div>
                        <div class="cc_top_one"><label>用户组:</label><span>{{$uInfo.groupName}}</span></div>
                        <div class="cc_top_one"><label>抽查通过率:</label><span>XXX</span></div>
                        <div class="clearfix"></div>
						<form action="{{$root_path}}statistics/shootDetail/{{$shootUserId}}">
                        ...
                        <div class="cc_top_one last_show"><label>状态:</label>
                           <div class="choice_count choice_box vocation">
                               <dl class="select">
                                   <select class="select3" name="shootStatus">
                                     <option value="">全部</option>
                                       {{foreach from=$statusList item=val key=key}}
                                       <option value="{{$key}}" {{if $key==$shootStatus}} selected="selected"{{/if}}>{{$val}}</option>
									   {{/foreach}}
                                  </select>
                               </dl>
                           </div>
